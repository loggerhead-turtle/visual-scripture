import { BOOKS, bookBySlug, entities, appearances, esc } from '../data.js';
...
export async function renderCast(el, params, focusId) {
  ...
  const sections = BOOKS.map(b => {
    ...
    return `<section class="cast-book-section" data-book="${b.slug}">
  ...
  }).join('') + (nowhere.length ? `<section class="cast-book-section"><h2>Quoted & Heavenly Voices</h2><div class="cast-grid">${nowhere.map(card).join('')}</div></section>` : '');

  const groupsHtml = `<section class="cast-book-section"><h2>Peoples &amp; Groups</h2>
    <div class="cast-grid">${ents.groups.map(g => card({ ch: g, apps: apps[g.id] || [] })).join('')}</div></section>`;

  el.innerHTML = `
  <div class="wrap">
    <div class="eyebrow">The cast</div>
    <h1 class="page-title">Characters of the Book of Mormon</h1>
    <p class="section-intro">Every voice in the record as a visual card, grouped under the book where they first step onto the stage.
    Click a card for their story, family ties, and every chapter where they appear.</p>
    <div class="cast-controls">
      <input type="search" id="cast-search" placeholder="Search characters… (e.g. Abinadi)">
    </div>
  ...
  const openModal = id => {
    ...
    const chips = a.slice(0, 40).map(x => `<a class="pill link" href="#/read/${x.slug}/${x.c}">${bookBySlug[x.slug].name} ${x.c}</a>`).join('');
    ...
    const close = () => { back.remove(); if (location.hash.startsWith('#/character/')) history.replaceState(null, '', '#/cast'); };
    back.addEventListener('click', ev => {
      if (ev.target === back || ev.target.closest('.close')) close();
